<template>
	<div class="widget-card accent-sky">
		<WidgetHeader icon="🍻" :title="$t('hoesik.title')" />
		<p class="roulette-result" :class="{ spinning: isSpinning }">
			{{ result || $t('hoesik.empty') }}
		</p>
...
import { useHoesikRoulette } from '../../composables/useHoesikRoulette'

const { result, isSpinning, spin } = useHoesikRoulette()
</script>
